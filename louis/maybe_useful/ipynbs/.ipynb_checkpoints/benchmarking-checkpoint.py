...
import sys
sys.path.append('/Users/louis/Documents/Phd/PyPOTS/ipynbs')
sys.path.append('/Users/louis/Documents/Phd/PyPOTS')
...
# ==========================
def benchmark_models(missing_rates, X, n_epochs=600, results_file="benchmark_results_march_latent_size_6_sampling_from_x.json", dataset_name = None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for n in range(n_runs):
        for p in missing_rates:
                ...
                for DAE in [True, False]:
                    for pre_impute in [True, False]:
                        for detach_for_kl in [False]:
                            ...
                            key = f"GP_UAE_{p}_{n}_alpha_{alpha}_sigma_{sigma}_model_p_{model_p}_detach_{detach_for_kl}_DAE_{DAE}_pre_impute{pre_impute}_latent_size_{latent_size}"
                            ...
                            torch.save(gpuae.model.backbone.encoder.state_dict(), 'model_weights_sampling_from_x/' +  key + '_encoder')
                            torch.save(gpuae.model.backbone.decoder.state_dict(), 'model_weights_sampling_from_x/' +  key + '_decoder')
            ...
            for alpha in [1, 0.1, 0.01]:
                ...
                key = f"Posterior_consistency_{p}_{n}_alpha_{alpha}_latent_size_{latent_size}"
                ...
                gpvae_posterior = train_model(X_train, X_test, X_original, epochs=n_epochs, posterior_consistency=True, alpha = alpha, latent_size = latent_size)

                training_time = time.time() - start_time

                results[key] = evaluate_model_fullmetrics(gpvae_posterior, X_test.copy(), X_original.copy(), missing_mask), training_time
                save_results(results, results_file)
                torch.save(gpvae_posterior.model.backbone.encoder.state_dict(), 'model_weights_sampling_from_x/' + key + '_encoder')
                torch.save(gpvae_posterior.model.backbone.decoder.state_dict(), 'model_weights_sampling_from_x/' +  key + '_decoder')
