...
def mean_impute(X):
    ...
    X_imputed = X.copy()
    mask = (X_imputed == 0)  # Assuming 0 represents missing values
    #print(np.nanmean(np.where(mask, np.nan, X_imputed), axis=(0, 1)).shape)
    feature_means = np.nanmean(np.where(mask, np.nan, X_imputed), axis=(0, 1))[None, None, :]
...
def knn_impute(X, k=5):
    ...
    X_flat = X.reshape(-1, X.shape[-1])  # Flatten time-series data for kNN
...
def missforest_impute(X):
    ...
    X_flat = X.reshape(-1, X.shape[-1])  # Flatten time-series data
    imputer = MissForest()
    X_imputed_flat = imputer.fit_transform(X_flat)
    return X_imputed_flat.reshape(X.shape)
...
def train_model(X_train, 
    ...
    optimizer = Adam(weight_decay = 0.005) #ReduceLROnPlateau(patience = 100, factor = 0.8)
    ...
    model = model_cls(
        ...
        beta=1.0, K=1, encoder_sizes=(16, 32), decoder_sizes=(32, 16),
        optimizer=optimizer, patience = 30
    )#.to(device)

    if model_cls == GP_UAE:
        X_ = X_train
        imputer = KNNImputer(n_neighbors=2)
        imputer.fit(X_.reshape(-1,X_.shape[-1]))
    ...
    #print(X_train)
    dataset_train = {'X': X_train, 'missing_mask': X_train!=0}
    dataset_val = {'X': X_test, 'X_ori': X_original, 'missing_mask': X_test!=0}
...
def evaluate_model_fullmetrics(model, X_test, X_original, missing_mask=None):
    ...
    try:
        ...
        X_recon = model  # Model directly returns an imputed array
        X_recon_mean = model
        use_latent_space = False  # Skip latent space evaluations
    except ValueError:
        print("⚠️ Model has diverged, returning zeros for all metrics.")
        return {
            "observed": {"rmse": 0., "nll": 0., "elbo": 0., "coverage": 0., "uncertainty_error_corr": 0.},
            "missing": {"rmse": 0., "nll": 0., "elbo": 0., "coverage": 0., "uncertainty_error_corr": 0.},
            "latent": {"latent_ks": 0.},
...
# ==========================
def benchmark_models(missing_rates, X, n_epochs = 300):
    ...
    for p in missing_rates:
        for n in range(n_runs):
            ...
            alpha, beta, lr, noise_std, noise_sigma = 1., 0.001, 5*1e-4, .5, 1e-2
            gpvae_alpha1 = train_model(X_train, X_test, X_original, epochs=n_epochs, alpha = alpha, beta = beta, lr = lr, noise_std = noise_std, noise_sigma = noise_sigma)
            top1 = time.time()    
            training_time_gpvae = top1 - start_time

            results[f"GP_VAE_{p}_{n}_alpha_{alpha}_beta_{beta}_lr_{lr}_noise_std_{noise_std}_noise_sigma_{noise_sigma}"] = evaluate_model_fullmetrics(gpvae_alpha1, X_test.copy(), X_original.copy(), missing_mask), training_time_gpvae

            #gpvae = train_model(X_train, epochs=n_epochs, alpha = 1e0)

            top1 = time.time()
            gpvae_posterior = train_model(X_train, X_test, X_original, epochs=n_epochs, posterior_consistency=True, alpha = 1e-1)
            top2 = time.time()
            training_time_gpvae_posterior = top2 - top1

            alpha, beta, lr, noise_std, noise_sigma = 1., 0.0001, 5*1e-3, .5, 1e-3
            gpvae_alpha2 = train_model(X_train, X_test, X_original, epochs=n_epochs, alpha = alpha, beta = beta, lr = lr, noise_std = noise_std, noise_sigma = noise_sigma)

            results[f"GP_VAE_{p}_{n}_alpha_{alpha}_beta_{beta}_lr_{lr}_noise_std_{noise_std}_noise_sigma_{noise_sigma}"] = evaluate_model_fullmetrics(gpvae_alpha2, X_test.copy(), X_original.copy(), missing_mask), training_time_gpvae


            # Evaluate Baseline Methods
            results[f"Mean_Imputation_{p}_{n}"] = evaluate_model_fullmetrics(mean_impute(X_test), X_test.copy(), X_original.copy(), missing_mask), np.nan
            results[f"KNN_Imputation_{p}_{n}"] = evaluate_model_fullmetrics(knn_impute(X_test), X_test.copy(), X_original.copy(), missing_mask), np.nan
            # results[f"MissForest_Imputation_{p}"] = evaluate_model_fullmetricsmissforest_impute(X_test), X_test, X_original, missing_mask)
            results[f"MICE_Imputation_{p}_{n}"] = evaluate_model_fullmetrics(mice_impute(X_test), X_test.copy(), X_original.copy(), missing_mask), np.nan

            gpvae_posterior1 = train_model(X_train, X_test, X_original, epochs=n_epochs, posterior_consistency=True, alpha = 1e-2)


            #results[f"GP_VAE_{p}"] = evaluate_model_fullmetrics(gpvae, X_test.copy(), X_original.copy(), missing_mask), training_time_gpvae
            results[f"Posterior_Consistency_{p}_{n}"] = evaluate_model_fullmetrics(gpvae_posterior, X_test.copy(), X_original.copy(), missing_mask), training_time_gpvae_posterior
            results[f"Posterior_Consistency1_{p}_{n}"] = evaluate_model_fullmetrics(gpvae_posterior1, X_test.copy(), X_original.copy(), missing_mask), training_time_gpvae_posterior
...
if __name__ == "__main__":
    ...
    # Run Benchmarking
    results = benchmark_models(missing_rates, X, n_epochs = 3000)
    ...
    with open(RESULTS_FILE, "w") as f:
        for key, metrics_tuple in results.items():
            ...
            f.write(f"  Latent KS: {format_metric(latent_ks)}\n")
