...
def train_model(X_train, 
    ...
    optimizer = Adam(lr = lr, 
                    weight_decay = 0.05) #ReduceLROnPlateau(patience = 100, factor = 0.8)
    ...
    model = model_cls(
        ...
        optimizer=optimizer, patience = 600
    )#.to(device)
    ...
    model.model.backbone.noise_sigma = noise_sigma
    model.model.backbone.use_mean = use_mean_z
    model.model.backbone.detach = detach
    model.model.backbone.noise_std = noise_std
    model.model.backbone.p = 0.5
...
def benchmark_models(missing_rates, X, n_epochs = 300):
    ...
    for p in missing_rates:
        for n in range(n_runs):
            ...
            gpvae_alpha1 = train_model(X_train, epochs=n_epochs, alpha = alpha, beta = beta, lr = lr, noise_std = noise_std, noise_sigma = noise_sigma)
            top1 = time.time()    
            ...
            top1 = time.time()
            gpvae_posterior = train_model(X_train, epochs=n_epochs, posterior_consistency=True, alpha = 1e-1)
            top2 = time.time()
            training_time_gpvae_posterior = top2 - top1

            alpha, beta, lr, noise_std, noise_sigma = 1., 0.0001, 5*1e-3, .5, 1e-3
            gpvae_alpha2 = train_model(X_train, epochs=n_epochs, alpha = alpha, beta = beta, lr = lr, noise_std = noise_std, noise_sigma = noise_sigma)
            ...
            gpvae_posterior1 = train_model(X_train, epochs=n_epochs, posterior_consistency=True, alpha = 1e-2)
