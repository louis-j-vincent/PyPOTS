    ...
    def fit_kernel(self, training_loader, val_loader=None, training_iter=20):
        ...
        for epoch in range(training_iter):
            ...
            for training_step, data in tqdm(enumerate(training_loader), desc=f"Epoch {epoch+1}/{training_iter}"):
                ...
                z_star_var = z_star_var.clamp(min=0.)**.5  # Avoid NaN variance
            ...
            if val_loader is not None:
                ...
                with torch.no_grad():  # No gradient updates for validation
                    for val_data in val_loader:
                        ...
                        z_samples_val = torch.randn((n_samples,) + z_star_val.shape) * torch.sqrt(z_star_var_val) + z_star_val

                        # Reconstruct validation data
                        x_recon_val = self.decode(z_samples_val).mean.mean(axis=0)
                        x_recon_val += torch.rand(x_recon_val.shape) * 1e-2  # Add noise
                        val_l2_error = (x_recon_val - x_val).pow(2)[(x_val != 0)].clip(max=10)

                        # Compute validation loss
                        val_loss = val_l2_error.mean()
                        #val_loss += self.prior_for_kernel_params(kernel_params_val) * alpha  # Regularization
