...
class _GP_UAE(nn.Module):
    ...
    def forward_for_GP(self, inputs, training=True, n_sampling_times=10, use_GP=False, gp = None):
        ...
        if training:
            ...
            kernel_params = self.gp.update_kernel_params(X_corr).detach()
    ...
    def forward_for_GP(self, inputs, training=True, n_sampling_times=10, use_GP=False, gp = None):
        ...
        if training:
            ...
            #recon_loss = self.gp.loss(X_corr, qz_x_corr)
    ...
    def recon_loss(self, X, X_corr, X_recon, eps = 1e-5):
        ...
        mask_hidden = missing_mask & (~missing_mask_corr)
        recon_loss_observed = (X[None] - X_recon).pow(2)[missing_mask_corr.unsqueeze(0).repeat(X_recon.shape[0],1,1,1)].mean()
        recon_loss_imputed = (X[None] - X_recon).pow(2)[mask_hidden.unsqueeze(0).repeat(X_recon.shape[0],1,1,1)].mean()

        loss = recon_loss_observed * alpha + (1 - alpha) * recon_loss_imputed
