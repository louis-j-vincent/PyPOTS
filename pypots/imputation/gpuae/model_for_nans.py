...
class GP_UAE(BaseNNImputer):
        ...
        try:
            ...
            for epoch in range(1, self.epochs + 1):
                ...
                if self.train_gp and epoch%10==0:
                    self._fit_kernel(training_loader, val_loader, training_iter = 1)  # train the model on the dataset
    ...
    def _train_kernel(
        ...
        try:
            ...
            for epoch in range(1, self.epochs + 1):
                ...
                #self.model.backbone.temperature = epoch / (self.epochs + 1)
                epoch_train_loss_collector = []
                for idx, data in tqdm(enumerate(training_loader)):
                    training_step += 1
                ...
                if val_loader is not None:
                    ...
                    with torch.no_grad():
                        for idx, data in enumerate(val_loader):
                            ...
                            print(imputed_data.shape, inputs['X_ori'].shape)
                            imputation_mse = (
                                calc_mse(
                                    imputed_data,
                                    inputs["X_ori"],
                ...
                with torch.no_grad():
                    ...
                    qz_x = self.model.backbone.encode(X, missing_mask)
                    kernel_params = self.gp.update_kernel_params(X)
                    q_star = self.gp.correct_qz_x_with_gp(qz_x, kernel_params)

                    self.gp.plot_latent_GP_regression(q_star, qz_x, X)

            # Print Training and Validation Loss
                if val_loader:
                    print(f"Epoch {epoch+1}/{self.epochs}: Training Loss = {mean_train_loss:.6f}, Validation Loss = {mean_val_loss:.6f}")
                else:
                    print(f"Epoch {epoch+1}/{self.epochs}: Training Loss = {mean_train_loss:.6f}")
    ...
    def impute_with_gp(self,
        ...
        x = inputs['X']
        if add_mcar:
            x_corr = mcar(torch.clone(x), self.p)
            x_corr[x_corr!=x_corr] = 0.
            qz_x = self.model.encode(x_corr, training=False, n_sampling_times=n_sampling_times)
            x = x_corr
        ...
        if self.use_gp or gp:
            ...
            imputed_data = self.gp.infer(qz_x, x)
