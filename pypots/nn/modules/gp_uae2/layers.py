...
class GpvaeEncoder(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        if self.pre_impute:
            x_imputed = self.pre_imputer.transform(np.array(x_reshaped.detach().numpy()))
            x_reshaped[~mask] = torch.Tensor(x_imputed).float().to(x_reshaped.device)[~mask]
        else:
...
class GpvaeEncoder_cnn(nn.Module):
    ...
    def __init__(self, input_size, z_size, hidden_sizes=(128, 128), window_size=24):
        del input_size, window_size
        super().__init__()
        self.z_size = int(z_size)
        h = tuple(hidden_sizes) if len(hidden_sizes) > 0 else (64, 128)
        c1 = max(16, int(h[0]))
        c2 = max(16, int(h[1] if len(h) > 1 else h[0] * 2))
        c3 = c2
        fc_hidden = max(64, int(h[-1] * 2))
        self.conv = nn.Sequential(
            nn.Conv2d(2, c1, kernel_size=3, stride=2, padding=1),  # 28 -> 14
            nn.ReLU(),
            nn.Conv2d(c1, c2, kernel_size=3, stride=2, padding=1),  # 14 -> 7
            nn.ReLU(),
            nn.Conv2d(c2, c3, kernel_size=3, stride=1, padding=1),  # 7 -> 7
            nn.ReLU(),
        )
        ...
        self.mu_layer = nn.Linear(fc_hidden, self.z_size)
        self.logvar_layer = nn.Linear(fc_hidden, self.z_size)

    def forward(self, x, mask=None):
        if mask is None:
            raise ValueError("`mask` is required for GpvaeEncoder_cnn.")
        if x.ndim != 4 or mask.ndim != 4:
            raise ValueError(f"Expected x/mask to be 4D (B,T,28,28), got {x.shape} and {mask.shape}.")

        batch_size, time_length, h, w = x.shape
        x_bt = x.reshape(batch_size * time_length, 1, h, w)
        m_bt = mask.reshape(batch_size * time_length, 1, h, w)
        xm = torch.cat([x_bt, m_bt], dim=1)

        features = self.conv(xm).reshape(batch_size * time_length, -1)
        hidden = self.fc(features)
        mu = self.mu_layer(hidden).view(batch_size, time_length, self.z_size)
        logvar = self.logvar_layer(hidden).view(batch_size, time_length, self.z_size)

        std = torch.exp(0.5 * logvar).clamp(min=1e-4, max=10.0)
        z_dist = torch.distributions.Normal(loc=mu, scale=std)
        return torch.distributions.Independent(z_dist, 1)
...
class GpvaeDecoder_cnn(nn.Module):
    """CNN decoder that reconstructs image sequences as Normal distributions."""

    def __init__(self, input_size, output_size, hidden_sizes=(256, 256)):
        del output_size
        super().__init__()
        h = tuple(hidden_sizes) if len(hidden_sizes) > 0 else (128, 64)
        fc_hidden = max(64, int(h[0] * 2))
        c1 = max(16, int(h[0]))
        c2 = max(16, int(h[1] if len(h) > 1 else h[0] // 2))
        stem = max(c1, c2)
        self.fc = nn.Sequential(
            nn.Linear(input_size, fc_hidden),
            nn.ReLU(),
            nn.Linear(fc_hidden, stem * 7 * 7),
            nn.ReLU(),
        )
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(stem, c1, kernel_size=4, stride=2, padding=1),  # 7 -> 14
            nn.ReLU(),
            nn.ConvTranspose2d(c1, c2, kernel_size=4, stride=2, padding=1),  # 14 -> 28
            nn.ReLU(),
    ...
    def forward(self, z):
        if z.ndim != 3:
            raise ValueError(f"Expected latent shape (B,T,Z), got {z.shape}.")
        batch_size, time_length, latent_dim = z.shape
        z_bt = z.reshape(batch_size * time_length, latent_dim)
        h = self.fc(z_bt)
        stem = h.shape[-1] // (7 * 7)
        h = h.view(batch_size * time_length, stem, 7, 7)
        mu = self.deconv(h).view(batch_size, time_length, 28, 28)
        mu = torch.sigmoid(mu)
        var = torch.full_like(mu, 0.3)
        return torch.distributions.Normal(mu, var)
