...
class BackboneGP_UAE(nn.Module):
    ...
    def nll(
        ...
        if self.compensate: # this means we sample from the corrupted x !
            ...
            compensated_nll = nll * (density_quotient) ** self.r # r should be between 0 and 1
        ...
        if mean_z:
            ...
            nll = compensated_nll * RECON_WEIGHT + nll_mean * (1 - RECON_WEIGHT)
        else:
            nll = compensated_nll
    ...
    def kl(self, qz_x: Any, qz_x_corrupted: Any, eps: float = EPSILON) -> torch.Tensor:
        ...
        # if 
        log_prob_qz_x_corrupted_for_mu_z = - 0.5 * (torch.log(2 * torch.tensor(np.pi, device=self.device) * var_z_corrupted)).sum(axis = -1)  #should be this qz_x_corrupted.log_prob(mu_z).clip(min = 0)
        kl_mean_z = log_prob_qz_x_corrupted_for_mu_z - torch.log((RECON_WEIGHT * torch.exp(qz_x.log_prob(mu_z)) + (1 - RECON_WEIGHT)))

        #print(kl_mean_z.mean(), qz_x_corrupted.log_prob(mu_z).mean(), qz_x.log_prob(mu_z).mean())

        return RECON_WEIGHT * kl.mean(axis = -1) + (1 - RECON_WEIGHT) * kl_mean_z / kl.shape[-1]
