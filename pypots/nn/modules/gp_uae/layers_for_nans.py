...
class GpvaeEncoder(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        mask =  torch.clone(mask.view(batch_size * time_length, input_size)).to(self.device)
        #mask = (x_reshaped!=x_reshaped).to(self.device)
        # mask = (x_reshaped!=0).to(self.device)
        # replace missing vals by random vals
        x_reshaped[~mask] = torch.rand(size = x_reshaped[~mask].shape).to(self.device)

        #print(torch.isnan(x_reshaped).float().mean(), torch.isnan(x_reshaped[mask]).float().mean(), torch.isnan(x_reshaped[~mask]).float().mean())
        ...
        assert not torch.isnan(mapped).any(), print('Nans in output of encoder', x_reshaped, x_reshaped[mask], x_reshaped[~mask])
