...
class BackboneGP_UAE(nn.Module):
    ...
    def forward(self, X: torch.Tensor, missing_mask: torch.Tensor, training = True) -> torch.Tensor:
        ...
        try:
            elbo = nll + kl * self.kl_weight + prior_loss * self.beta 
        except:
    ...
    def elbo(
        ...
        qz_x_corrupted = self.encode(X_corrupted, missing_mask)
        nll_elbo = self.nll(X, X_corrupted, qz_x, qz_x_corrupted, mean_z=False).mean()
        nll_mean = self.nll(X, X_corrupted, qz_x, qz_x_corrupted, mean_z=True).mean()
    ...
    def nll(
        ...
        #z = qz_x.rsample() if not mean_z else qz_x.mean
        ...
        compensated_nll = torch.where(condition_mask, nll, torch.zeros_like(nll))
        nll = compensated_nll.sum(axis=2) / (condition_mask.sum(axis=2) + eps)
    ...
    def kl(self, qz_x: Any, qz_x_corrupted: Any, eps: float = EPSILON) -> torch.Tensor:
        ...
        if self.detach_for_kl:
            mu_z, var_z = mu_z, var_z.detach()

        if self.DAE:
            mu_q_tilde = alpha * mu_z + (1 - alpha) * mu_z_corrupted
        else:
            mu_q_tilde = mu_z
        var_q_tilde = alpha**2 * var_z  

        kl = 0.5 * (
            (torch.log(var_z_corrupted) - torch.log(var_q_tilde))
            + (var_q_tilde + (mu_z_corrupted - mu_q_tilde).pow(2)) / (var_z_corrupted + eps) - 1)

        #kl = 0.5 * (
        #    (torch.log(var_q_tilde) - torch.log(var_q_tilde))
        #    + (var_q_tilde + (mu_z_corrupted - mu_q_tilde).pow(2)) / (var_z_corrupted + eps) - 1)
    ...
    def kl(self, qz_x, qz_x_corrupted, eps = 1e-3):
        ...
        mu_z, mu_z_corrupted = qz_x.mean, qz_x_corrupted.mean
        var_z, var_z_corrupted = qz_x.variance.detach(), qz_x_corrupted.variance
