...
def plot_latent_series_and_reconstruction(
    ...
    z_var  = torch.nan_to_num(qz_x.variance[0].clip(min=1e-3),1e-3).detach().cpu().numpy()    # shape: [time, latent_dim]
    z_std = torch.sqrt(qz_x.variance[0].clip(min=1e-3)).detach().cpu().numpy()
    ...
    for dim in range(latent_dim):
        ...
        #print(z_var)
        #print(np.sqrt(z_var[:, dim]))
        plt.fill_between(
            t,
            z_mean[:, dim] - z_std[:, dim],
            z_mean[:, dim] + z_std[:, dim],
            alpha=0.2,
